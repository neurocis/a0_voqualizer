...
def test_voqualizer_page_static_shell_debug_marker():
    js = read(JS)
    assert "m8-tts-suppress-store-listeners" in js
...
def test_voqualizer_preloads_last_monologue_result():
    ...
    for token in [
        ...
        assert token in js, token
    assert "m8-tts-suppress-store-listeners-2026-05-29-48" in html
    assert "m8-tts-suppress-store-listeners-2026-05-29-48" in css
