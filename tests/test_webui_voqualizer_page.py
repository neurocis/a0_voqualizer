...
def test_voqualizer_page_static_shell_debug_marker():
    js = read(JS)
    assert "m8-tts-vu-asr-style-continuous" in js
...
def test_voqualizer_preloads_last_monologue_result():
    ...
    for token in [
        ...
        assert token in js, token
    assert "m8-tts-vu-asr-style-continuous-2026-06-04-82" in html
    assert "m8-tts-vu-asr-style-continuous-2026-06-04-82" in css
...
def test_voqualizer_tts_suppresses_ack_fallback_after_live_push():
    ...
    assert "live_push_already_streamed" in js
    assert "ack_suppressed_at=" in js
...
def test_voqualizer_store_suppresses_tts_listeners_for_standalone():
    ...
    assert "suppressed_store_tts_ack" in conv
    assert "suppressed_store_tts_done" in conv
...
def test_voqualizer_cache_busts_conversation_mode_import():
    js = read(JS)
    assert "m8-tts-vu-asr-style-continuous" in js
    assert "conversation-mode.js?v=m8-tts-vu-asr-style-continuous-2026-06-04-82" in js
    assert "store_import_cache=m8-tts-vu-asr-style-continuous-2026-06-04-82" in js
    conv = read(CONVERSATION_MODE)
    assert "if (!this._suppressTts)" in conv
    assert "suppressed_store_tts_chunk" in conv
...
def test_voqualizer_realtime_ws_prompt_submit_path():
    ...
    assert "promptSubmitTransport = 'http_fallback'" in js
    assert "ws_prompt_transport=" in js
    assert "voqualizer_text_prompt" in py
    assert "async def _handle_text_prompt" in py
    assert "ws_text_prompt" in py
    assert "realtime_session" in py
    assert "\"protocol_version\": \"1.3\"" in py
    assert "context.communicate" in py
    assert "mq.log_user_message" in py
...
def test_voqualizer_authoritative_tts_stream_only():
    ...
    assert "authoritative_stream_waiting" in js
    assert "stale_generation" in js
    assert "tts_mode=" in js
    assert "active_generation=" in js
    assert "authoritative_tts_stream" in py
    assert "\"protocol_version\": \"1.3\"" in py
    assert "active_generation_id" in finalizer
    assert 'request_metadata["generation_id"]' in finalizer


def test_voqualizer_authoritative_tts_single_session_route():
    js = read(JS)
    finalizer = read(ROOT / "helpers" / "agent_finalizer.py")
    ws = read(ROOT / "api" / "ws_voqualizer.py")
    assert "m8-tts-vu-asr-style-continuous" in js
...
def test_voqualizer_collapses_response_tool_json_in_transcript():
    js = read(JS)
    html = read(HTML)
    css = read(CSS)
    assert "m8-tts-vu-asr-style-continuous" in js
    ...
    assert "parsed.tool_args?.text" in js
    assert "setBubbleBodyText" in js
    assert "lastCollapsedResponseJsonAt" in js
    assert "collapsed_json_at=" in js


def test_voqualizer_fix_kind_reference_and_ignore_thank_you_asr():
    js = read(JS)
    html = read(HTML)
    css = read(CSS)
    assert "m8-tts-vu-asr-style-continuous" in js
    ...
    assert "lastIgnoredAsrFinalText" in js
    assert "ignored_asr_final=" in js


def test_voqualizer_asr_store_does_not_own_tts_stream():
    js = read(JS)
    conv = read(ROOT / "webui" / "conversation-mode.js")
    ws = read(ROOT / "api" / "ws_voqualizer.py")
    finalizer = read(ROOT / "helpers" / "agent_finalizer.py")
    ...
    assert "tts_suppressed" in ws
    assert "tts_disabled_or_suppressed" in finalizer
    assert "tts_disabled_or_suppressed" in chunker


def test_voqualizer_asr_init_remains_enabled_when_tts_suppressed():
    js = read(JS)
    conv = read(ROOT / "webui" / "conversation-mode.js")
    ws = read(ROOT / "api" / "ws_voqualizer.py")
    assert "m8-tts-vu-asr-style-continuous" in js
    ...
    assert "voqualizer_init_missing_session_token" in conv
    assert "asr_enabled" in ws


def test_voqualizer_ignores_blank_silence_inaudible_asr():
    js = read(JS)
    html = read(HTML)
    css = read(CSS)
    assert "m8-tts-vu-asr-style-continuous" in js
...
def test_voqualizer_asr_provider_reset_debug_after_background_error():
    js = read(JS)
    html = read(HTML)
    css = read(CSS)
    ws = read(ROOT / "api" / "ws_voqualizer.py")
    ...
    assert 'asr_background_error_suppressed_count' in ws
    assert 'asr_bg_errors=' in js
    assert 'asr_provider_resets=' in js
    assert 'asr_empty_suppressed=' in js
    assert 'asr_stale_ignored=' in js


def test_voqualizer_prompt_clear_button():
    html = read(HTML)
    css = read(CSS)
    js = read(JS)
    assert "m8-tts-vu-asr-style-continuous" in js
    ...
    assert '.voq-prompt-clear' in css
    assert 'padding: 14px 48px 14px 16px' in css
    assert 'function updatePromptClearButton' in js
    assert 'function clearPromptText(state)' in js
    assert 'lastPromptClearAt' in js
    assert "clear.addEventListener('click', () => clearPromptText(state));" in js


def test_voqualizer_ignores_throat_clearing_asr_artifacts():
    html = read(HTML)
    css = read(CSS)
    js = read(JS)
    assert "m8-tts-vu-asr-style-continuous" in js
    ...
    assert "throat clearing" in js
    assert "false_positive_silence_or_filler" in js
    assert "lastIgnoredAsrFinalText" in js
    assert "recordIgnoredAsrFinal" in js


def test_voqualizer_removes_tts_v_badge():
    html = read(HTML)
    css = read(CSS)
    js = read(JS)
    assert "m8-tts-vu-asr-style-continuous" in js
    ...
    assert 'voqualizer-button-badge' not in speaker_block
    assert '>V<' not in speaker_block


def test_voqualizer_tts_green_vu_meter():
    html = read(HTML)
    css = read(CSS)
    js = read(JS)
    assert "m8-tts-vu-asr-style-continuous" in js
    ...
    assert 'rgba(46, 204, 113' in css
    assert 'function bytesToTtsVuPercent' in js
    assert 'function updateTtsVu(bytes, playbackDelayMs = 0, durationMs = 180)' in js
    assert 'function clearTtsVu()' in js
    assert 'scheduled_pcm_playback' in js
    assert 'updateTtsVu(aligned, Math.max(0, (startAt - ctx.currentTime) * 1000), buffer.duration * 1000)' in js
    assert 'tts_vu=' in js
    assert 'tts_vu_hold=' in js
    assert 'tts_vu_delay=' in js
    assert 'tts_vu_source=' in js
    assert 'lastTtsVuHoldMs' in js
    assert 'ttsVuClearTimer' in js
    assert 'ttsVuDecayTimer' in js
    assert 'function decayTtsVu()' in js
    assert 'lastTtsVuDecayAt' in js
    assert 'lastTtsVuSource' in js
    assert 'lastTtsVuPlaybackDelayMs' in js
    assert 'setTtsVuVisual' in js
    assert 'stopTtsVuTimers' in js
    assert 'ttsVuEnvelopeTimer' in js
    assert 'function getTtsVuElement' in js
    assert 'speaker.dataset.ttsVuActive' in js
    assert 'lastTtsVuTargetFound' in js
    assert 'lastTtsVuElementFound' in js
    assert 'tts_vu_target=' in js
    assert 'tts_vu_element=' in js
    assert 'conic-gradient' in css
    assert '.voqualizer-tts-vu::after' in css
    assert 'voqualizer-tts-vu-bar' in html
    assert '.voqualizer-tts-vu-bar' in css
    assert '.voqualizer-speaker.voqualizer-tts-playing .voqualizer-tts-vu' in css
    assert 'radial-gradient(circle at 50% calc(100% - 8px)' in css
    assert 'translateX(-10px)' in css
    assert 'pcm_playback' in js
    assert 'tts_vu_visual_active=' in js
    assert 'tts_vu_rect=' in js
    assert 'ensureTtsVuIdleVisual' in js
    assert 'voqualizer-tts-vu-idle' in js
    assert 'tts_vu_idle=' in js
    assert 'tts_vu_bars=' in js
    assert 'startTtsVuPlayback' in js
    assert 'stopTtsVuLoop' in js
    assert 'ttsVuActiveUntilMs' in js
    assert 'tts_vu_active=' in js
    assert 'tts_vu_continuous=' in js
    assert 'ttsVuPlaybackEndMs' in js
    assert 'tts_vu_segments=' in js
    assert 'tts_vu_sched_level=' in js
    assert '#voqualizer-speaker-button[aria-pressed="true"] .voqualizer-tts-vu' in css
    assert '900' in js
    assert '650' in js
