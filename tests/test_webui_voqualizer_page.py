...
def test_voqualizer_page_static_shell_debug_marker():
    js = read(JS)
    assert "m8-tts-vu-longer-linger" in js
...
def test_voqualizer_preloads_last_monologue_result():
    ...
    for token in [
        ...
        assert token in js, token
    assert "m8-tts-vu-longer-linger-2026-06-02-75" in html
    assert "m8-tts-vu-longer-linger-2026-06-02-75" in css
...
def test_voqualizer_cache_busts_conversation_mode_import():
    js = read(JS)
    assert "m8-tts-vu-longer-linger" in js
    assert "conversation-mode.js?v=m8-tts-vu-longer-linger-2026-06-02-75" in js
    assert "store_import_cache=m8-tts-vu-longer-linger-2026-06-02-75" in js
    conv = read(CONVERSATION_MODE)
    assert "if (!this._suppressTts)" in conv
    assert "suppressed_store_tts_chunk" in conv
...
def test_voqualizer_tts_green_vu_meter():
    ...
    assert 'function updateTtsVu(bytes)' in js
    assert 'function clearTtsVu()' in js
    assert 'updateTtsVu(bytes)' in js
    assert 'tts_vu=' in js
