...
def test_voqualizer_page_static_shell_debug_marker():
    js = read(JS)
    assert "m8-tts-vu-playback-queue" in js
...
def test_voqualizer_preloads_last_monologue_result():
    ...
    for token in [
        ...
        assert token in js, token
    assert "m8-tts-vu-playback-queue-2026-06-04-80" in html
    assert "m8-tts-vu-playback-queue-2026-06-04-80" in css
...
def test_voqualizer_cache_busts_conversation_mode_import():
    js = read(JS)
    assert "m8-tts-vu-playback-queue" in js
    assert "conversation-mode.js?v=m8-tts-vu-playback-queue-2026-06-04-80" in js
    assert "store_import_cache=m8-tts-vu-playback-queue-2026-06-04-80" in js
    conv = read(CONVERSATION_MODE)
    assert "if (!this._suppressTts)" in conv
    assert "suppressed_store_tts_chunk" in conv
...
def test_voqualizer_tts_green_vu_meter():
    ...
    assert '@keyframes voq-tts-vu-bars' in css
    assert 'scheduled_pcm_playback_queue' in js
    ...
    assert 'scheduleTtsVuSegment' in js
    assert 'ensureTtsVuPlaybackLoop' in js
    assert 'ttsVuScheduledSegments' in js
    assert 'tts_vu_queue=' in js
